    ...
    for attempt in range(5):
        try:
            req = urllib.request.Request(
                url, headers={"User-Agent": "OncoMatchmaker-snapshot/0.1"}
            )
...
def build(output):
    ...
    while True:
        params = {
            "query.cond": QUERY,
            "filter.overallStatus": ",".join(manifest["statuses"]),
            "pageSize": 1000,
            "countTotal": "true",
            "format": "json",
        }
        ...
        print(
            "Page {}: {} / {} studies".format(
                len(manifest["pages"]),
                db.execute("SELECT count(*) FROM studies").fetchone()[0],
                expected,
            ),
            flush=True,
        )
    ...
    for nct_id, raw in db.execute("SELECT nct_id, raw_json FROM studies"):
        count = len(
            json.loads(raw)["protocolSection"]
            .get("contactsLocationsModule", {})
            .get("locations", [])
        )
        if (
            db.execute(
                "SELECT count(*) FROM sites WHERE nct_id=?", (nct_id,)
            ).fetchone()[0]
            != count
        ):
    ...
    manifest.update(
        {
            ...
            "recruiting_site_count": db.execute(
                "SELECT count(*) FROM recruiting_sites"
            ).fetchone()[0],
            "site_status_counts": dict(
                db.execute(
                    "SELECT coalesce(status,'MISSING'),count(*) FROM sites GROUP BY status"
                )
            ),
            "study_type_counts": dict(
                db.execute(
                    "SELECT coalesce(study_type,'MISSING'),count(*) FROM studies GROUP BY study_type"
                )
            ),
            "country_count": db.execute(
                "SELECT count(DISTINCT country) FROM sites"
            ).fetchone()[0],
        }
    )
    ...
    for term in [
        ...
        missing = sorted(
            i
            for i in ids
            if not db.execute("SELECT 1 FROM studies WHERE nct_id=?", (i,)).fetchone()
        )
        manifest["coverage_audits"][term] = {
        ...
        json.dumps(
            {
                ...
                if k not in ("pages", "query", "condition_terms")
            },
            indent=2,
        )
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "output",
        type=Path,
        help="New snapshot directory; existing directories are never overwritten",
    )
