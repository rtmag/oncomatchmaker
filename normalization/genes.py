...
class HGNCRegistry:
    ...
    def synonyms(self, symbol: str) -> list[str]:
        """Return exact HGNC aliases/previous symbols for retrieval, never fuzzy terms."""
        record = self.approved.get(symbol.upper())
        if not record:
            return []
        values = record.get("alias_symbol", []) + record.get("prev_symbol", [])
        return sorted({value for value in values if value.upper() != symbol.upper()})
