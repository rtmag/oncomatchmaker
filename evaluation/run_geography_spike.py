...
from trials.geography import StaticCityResolver, nearest_recruiting_sites  # noqa: E402
...
def main():
    ...
    parser.add_argument(
        "--cities", type=Path, default=ROOT / "geography_test_cities.json"
    )
    parser.add_argument(
        "--scenarios", type=Path, default=ROOT / "geography_test_scenarios.json"
    )
    ...
    rendered = (
        json.dumps(
            run(args.database, args.cities, args.scenarios, args.limit), indent=2
        )
        + "\n"
    )
    if args.output:
