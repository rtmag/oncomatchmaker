...
export type PlotPoint = {
  ...
  coverage: number; bounds?: number[] | null; status: string; reviewed?: RankedTrial
}
export function buildPlotPoints(trials: RankedTrial[], landscape: ScreeningPoint[]): PlotPoint[] {
  const points = new Map<string, PlotPoint>(landscape.map(row => [row.nct_id, {
    ...
    plot_score: row.clinical_assessment?.score_version === "clinical-fit-v2" ? row.clinical_score ?? null : null,
  ...
  for (const row of trials) points.set(row.trial.nct_id, {
    ...
    plot_score: row.category !== "excluded" && row.match.score_version === "clinical-fit-v2" ? row.match.overall_score : null,
    ...
    status: row.category === "excluded" ? "excluded" : row.match.score_version === "clinical-fit-v2" ? "expert_reviewed" : "legacy_unscored", reviewed: row,
  })
