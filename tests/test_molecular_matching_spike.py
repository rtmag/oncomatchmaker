...
class MatchingSpikeTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with sqlite3.connect(database) as db:
                ...
                db.execute(
                    ...
                    ("NCT00000001", "Synthetic", "RECRUITING", "INTERVENTIONAL",
                     "TREATMENT", "Requires EGFR L858R", '["NSCLC"]', "2026-01-01",
                     "2026-01", "test-time", "https://example.test/NCT00000001"),
                )
    ...
    def test_every_profile_has_positive_and_negative_pressure_tests(self):
        candidates = matching_spike.load_json(matching_spike.SPIKE / "candidate_sets.json")
        self.assertTrue(all(len(identifiers) >= 3 for identifiers in candidates.values()))
        self.assertEqual(len(candidates), 4)

    def test_assessment_covers_each_pair_once_and_preserves_safety_controls(self):
        candidates = matching_spike.load_json(matching_spike.SPIKE / "candidate_sets.json")
        results = matching_spike.load_json(matching_spike.SPIKE / "chatgpt_assessments.json")
        assessed = [(row["case_id"], row["nct_id"]) for row in results["assessments"]]
        expected = [(case_id, nct_id) for case_id, ids in candidates.items() for nct_id in ids]
