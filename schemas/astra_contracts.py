    ...
    for collection in ("snv_indel", "copy_number", "fusions"):
        for index, finding in enumerate(biomarkers.get(collection, [])):
            ...
            if not isinstance(gene, str) or not gene.strip():
                raise ContractError(
                    f"profile.biomarkers.{collection}[{index}].gene is required"
                )
            if gene != gene.upper():
...
class ExpertAssessment:
    ...
    def from_dict(cls, value: Mapping[str, Any]) -> "ExpertAssessment":
        ...
        relationships = _list_of_strings(
            value["relationships"], "assessment.relationships"
        )
        ...
        for field in (
            ...
            list_fields[field] = tuple(
                _list_of_strings(value[field], "assessment." + field)
            )
        if not isinstance(value["trial_id"], str) or not value["trial_id"].startswith(
            "NCT"
        ):
            raise ContractError("assessment.trial_id must be an NCT identifier")
        if (
            not isinstance(value["reasoning_summary"], str)
            or not value["reasoning_summary"].strip()
        ):
